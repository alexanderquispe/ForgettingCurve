...
class HLR_model:
    ...
    def __init__( self, method, feature_columns, omit_h_term = False, lrate = .001, alpha_ = .01, lambda_ = .1, sigma_ = 1. ):
    ...
    def _find_p( self, row = None, h_hat = None, delta_ = None ):
    ...
    def _estimate_losses( self, row, delta_, p , h ):
        ...
        p_hat = self._find_p( h_hat = h_hat, delta_ = delta_ )
        slp   = ( p - p_hat ) ** 2
        slh   = ( h - h_hat ) ** 2
...
def read_data( df, method, omit_lexemes = False  ):
    ...
    if not omit_lexemes:
        ...
        feature_vars = ['right', 'wrong', 'bias'] + lexeme_columns + ( [ 'time' ] if method == 'lr' else [] )                          
